...
| [reference/insight_types_1.2.0/](reference/insight_types_1.2.0/) | **Partial historical reference snapshot** of the 1.2.0 design package: JSON schemas, response contract, fallback-signature registry, conformance-kit procedure, provenance. **8 of the 31 files named by the original source manifest are included**; the executable conformance runner and sample adapter are not shipped here. Source-archive verification does not certify this checkout or a host implementation. Reference material, not runtime — and the contract files are a 1.2.0 freeze, while the runtime ships 1.3.0. |
