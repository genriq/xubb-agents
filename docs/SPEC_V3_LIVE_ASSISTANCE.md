...
**Terminology:** "the host" is whatever application embeds this library; "the desktop host" is the embedding desktop application whose behaviour drove the observed failures. "The principal" is the human the swarm assists; "the counterpart" is whoever they are talking to. "Nature" = what an insight is; "urgency" = when it must be seen; "outcome" = what the principal did with it.
