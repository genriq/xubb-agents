...
**Typed reach (v2.8, `docs/SPEC_V2_8_TYPED_REACH.md`):** every shipped schema but `custom1` registers under `typed_v1` through a declared adapter — `insight_v1`, `flat_v2` (`default_v2`), `flat_v1` (`default`: candidate at the root plus `memory_updates`), `root_v2` (`v2_raw`, and `ui_control` / `widget_control` with their `ui_actions` sidecar named by the descriptor's `sidecar_instruction`); `custom1` fails typed registration with its `typed_unsupported_reason`, and a descriptor declaring `typed_v1` without a `typed_adapter` is a registration error. Framework-built segment entries carry `source_index` (the position in the host's `recent_segments` list, before trimming) and `timestamp`; accepted segment references are stamped with the engine-owned `EvidenceRef.source_index`; `HostInsightCapabilities.evidence_citations` exposes the citation material on every typed run. The isolated content path's instruction offers the envelope only. `AgentInsight.urgency_provided` is engine-owned provenance (explicit only). `await ContentTaskHandle.result()` returns after the capacity slot is released (`handle.released`). `PriorInsightRecord.correctable=False` removes a record from the offered targets (`target_not_correctable`). `AgentResponse.data_by_agent` attributes committed sidecars per agent on aggregated responses.
